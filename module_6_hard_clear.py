...
class Figure:
    ...
    def is_valid_sides(self, new_sides):  # валидация вводимых сторон
        if len(new_sides) != len(self.__sides):  # если кол-во не равно предыдущему
            print(f'you need to input {self.sides_count} sides, try again')
            return False
        if not all(isinstance(num, int) for num in new_sides):  # если числа в списке не являются целыми
            print(f'some of nums are not int, impossible to change')
            return False
        else:
            print('sides are changed')  # все принты исключительно для удобства тестирования
            return True
...
class Circle(Figure):
    ...
    def __len__(self):  # просто длина(периметр) круга
    ...
    def get_square_rad(self):  # площадь круга из радиуса (попробовала оба метода), чтобы понять. что радиус
        ...
        s = pi * self.__radius ** 2  # вот эта хрень мне не нравится (при переопределении сторон - не срабатывает)
        return s
...
class Cube(Figure):
    ...
    def is_valid_sides(self, new_sides):  # переписала метод валидации, только из-за первой строчки
        if len(new_sides) != 1:
            print(f'you need input only 1 side, cuz all sides in cubes are equal')
            return False
        if not all(isinstance(num, int) for num in new_sides):
            print(f'some of sides are not int, impossible to change params')
            return False
        else:
            print('sides are changed')
            return True
